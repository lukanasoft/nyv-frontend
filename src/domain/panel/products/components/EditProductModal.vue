<template>
    <div class="modal-card">
                ...
                <div class="photos-grid">
                    <div v-for="(file, index) in product.photos" :key="index" style="position: relative">
                        ...
                        <div v-else>
                            ...
                            <feather-icon 
                                class="is-clickable has-text-info" 
                                @click="removePhotoId(file, index)" 
                                icon="XIcon" 
                                style="position: absolute" 
                                size="20"
                            />
        ...
        <footer class="modal-card-foot has-background-white is-flex is-align-items-center is-justify-content-end">
            <b-button class="button-blue-dashboard is-primary" @click="uploadProduct">Editar producto</b-button>
        </footer>
    </div>
</template>
...
export default {
    ...
    async created() {
        ...
        this.product = JSON.parse(JSON.stringify(this.productState));
    },
    computed: {
        ...mapState({
            ...
            productState: state => state.Products.product
        }),
    },
    methods: {
        closeModal() {
        ...
        async removePhotoId(photo, index) {
            const result = await this.$swal({
                title: '¿Estas seguro que deseas eliminar esta foto?',
            ...
            if(result.isConfirmed) {
                await this.$store.dispatch('Products/deletePhoto', {photo, product: this.product});
                this.removePhoto(index);
                this.$store.commit('Products/UPDATE_PRODUCT', this.product);
            }
        },
        ...
        async uploadProduct() {
            ...
            formData.append('application', this.product.aplication);
            formData.append('importance', this.product.importance);
            formData.append('brand', this.product.brand_id);
            ...
            // send request
            try {
                await this.$store.dispatch('Products/editProduct', {formData, product: this.product});
                await this.$swal({
                ...
                this.$swal({
                    title: 'Error',
                    text: 'Ha ocurrido un error al editar el producto, intentalo otra vez',
                    icon: 'error',
                    showCancelButton: false,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
